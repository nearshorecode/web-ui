...
const items = [
  ...
  {
    image:
      'https://ew1rnkpr0125002.blob.core.windows.net/media/MeccaBingo/SlotsAndGames/Thumbnail/george-and-the-dragon-large.png',
    text: 'Example Test 4',
  },
  {
...
export default function createStories() {
  return storiesOf('Presenter', module)
    .add('Default', withInfo({ text: 'WIP: Presenter default' })(() => <Presenter items={items} />))

    .add(
      'Example Rotation',
      withInfo({ text: 'WIP: Rotation' })(() => <Presenter transitions={TransitionRotate} items={items} />)
    )

    .add(
      'Example Vertical Slide',
      withInfo({ text: 'WIP: Vertical Slide Animation' })(() => (
        <Presenter transitions={TransitionVerticalSlide} items={items} />
      ))
    )

    .add(
      'Example Mixed Transitions',
      withInfo({ text: 'WIP: Mixed Animation' })(() => <Presenter transitions={TransitionWhatever} items={items} />)
    )

    .add(
      'Example with Right bar',
      withInfo({ text: 'WIP: Mixed Animation' })(() => (
        <Presenter transitions={TransitionWhatever} items={items} rightBar={<button>Right Bar button</button>} />
      ))
    );
}
